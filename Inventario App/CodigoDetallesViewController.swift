        ...
        let task = URLSession.shared.dataTask(with: request) {data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    // Actualizar campos
                    print("dato: \(self.jsonData["info"][0]["nombre"])")
                    if (self.jsonData["info"][0]["nombre"].stringValue != ""){
                        self.codigoLabel.text = self.codigo
                        self.nombreLabel.text = self.jsonData["info"][0]["nombre"].stringValue
                        self.proveedorLabel.text = self.jsonData["info"][0]["proveedor"].stringValue
                        self.descripcionLabel.text = self.jsonData["info"][0]["descripcion"].stringValue
                        self.serieLabel.text = self.jsonData["info"][0]["num_serie"].stringValue
                        self.cantidadLabel.text = self.jsonData["info"][0]["cantidad"].stringValue
                        self.categoriaLabel.text = self.jsonData["info"][0]["categoria"].stringValue
                        self.empresaLabel.text = self.jsonData["info"][0]["empresa"].stringValue
                        self.responsableLabel.text = self.jsonData["info"][0]["responsable"].stringValue
                        self.ubicacionLabel.text = self.jsonData["info"][0]["ubicacion"].stringValue
                        self.idPedidoLabel.text = self.jsonData["info"][0]["id_pedido"].stringValue
                        //let empresa = self.jsonData["empresa"].stringValue
                        self.empresaImageView.image = UIImage(named: self.jsonData["info"][0]["empresa"].stringValue)
                        /*
                         switch empresa {
